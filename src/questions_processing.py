...
class QuestionsProcessor:
        ...
        if not hasattr(self, 'companies_df'):
            ...
            try:
                ...
                try:
                    ...
                    self.companies_df = pd.read_csv(self.subset_path, encoding='latin-1')
        
        found_companies = []
        company_names = sorted(self.companies_df['company_name'].unique(), key=len, reverse=True)
        
        # 首先尝试提取引号内的内容（用户可能输入了带引号的公司名称）
        quoted_names = re.findall(r'"([^"]+)"', question_text)
        if quoted_names:
            # 检查引号内的内容是否匹配 subset.csv 中的公司名称
            for quoted_name in quoted_names:
                # 精确匹配（去除首尾空格）
                quoted_name = quoted_name.strip()
                # 检查是否在 company_names 列表中（不区分大小写）
                for company in company_names:
                    if company.strip().lower() == quoted_name.lower():
                        if company not in found_companies:
                            found_companies.append(company)
                        break
        
        # 如果通过引号匹配没有找到，再尝试直接文本匹配
        if not found_companies:
            for company in company_names:
                escaped_company = re.escape(company)
                # 匹配公司名称，前后可以有引号或非单词字符
    ...
    def process_question(self, question: str, schema: str):
        ...
        if len(extracted_companies) == 0:
            # 尝试提取引号内的内容，给用户更友好的提示
            quoted_content = re.findall(r'"([^"]+)"', question)
            if quoted_content:
                raise ValueError(
                    f"未在数据集中找到公司名称 '{quoted_content[0]}'。"
                    f"请确保公司名称与数据集中的名称完全一致（包括大小写和标点符号）。"
                )
            else:
                raise ValueError(
                    "未在问题中找到公司名称。请确保问题中包含公司名称（用引号括起来），例如：\"公司名\"的问题内容"
                )
