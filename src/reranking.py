...
class LLMReranker:
    ...
    def get_rank_for_single_block(self, query, retrieved_document):
        ...
        if self.provider == "openai":
            ...
            rsp = self._call_dashscope_with_retry(messages)
    ...
    def _call_dashscope_with_retry(self, messages):
        """带重试机制的 DashScope API 调用
        
        网络异常时会自动重试，最多重试5次，采用指数退避策略（4秒到30秒）
        """
        try:
            return self.llm.Generation.call(
                model="qwen-turbo",
                messages=messages,
                temperature=0,
            ...
            error_msg = str(e)
            print(f"⚠️ DashScope API 调用异常: {error_type} - {error_msg[:200]}")
            raise
